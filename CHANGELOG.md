# Changelog

## [1.8.0](https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/compare/v1.7.5...v1.8.0) (2026-02-01)


### Features

* **auto-detect:** implement page detection scoring/grouping + popup integration ([6c962a9](https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/commit/6c962a994aaea3e2de882f06e69284078e9630f4))


### Bug Fixes

* **auto-detect:** improve URL-family grouping and prevent hex filename dedup collisions ([5b0b3ad](https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/commit/5b0b3ad0dd480b2ac68c2f41ab5c9fa145b63a63))
* **auto-detect:** persist default-on toggle and disable autoscan when unchecked ([8ab92da](https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/commit/8ab92da5f23dd5f2bee8e0fec11c16f0d70106e7))
* **download:** default to CBZ and reorder formats (CBZ → PDF → ZIP) ([685eb1b](https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/commit/685eb1bc1908764950bb4ed40cba7faec92598ca))

## [1.7.5](https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/compare/v1.7.4...v1.7.5) (2026-02-01)


### Bug Fixes

* **firefox:** generate AMO-compatible manifest from manifest.base.json ([180159d](https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/commit/180159ddad852d5d10914861344d3de8e19622b5))
* **firefox:** generate AMO-compatible manifest from manifest.base.json ([e603cc3](https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/commit/e603cc34bf1c1c590ea321c29d9bf9e4e03d4bfb))

## [1.7.4](https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/compare/v1.7.3...v1.7.4) (2026-02-01)


### Bug Fixes

* **firefox:** add data_collection_permissions for AMO validation ([94d20a7](https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/commit/94d20a757ec9a45b6a0b2cdc803c439c961ebe14))

## [1.7.3](https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/compare/v1.7.2...v1.7.3) (2026-02-01)


### Bug Fixes

* **firefox:** add gecko add-on id for MV3 AMO submission ([e94994a](https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/commit/e94994a9dcd8ff3b43016eb774d5c11677e0be8a))
* **firefox:** add gecko add-on id for MV3 AMO submission ([0a222df](https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/commit/0a222df3a5e9077267789961087d216b80dcd3f0))

## [1.7.2](https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/compare/v1.7.1...v1.7.2) (2026-01-31)


### Bug Fixes

* dedupe fetched images by src url to prevent duplicates in image … ([d4db1bf](https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/commit/d4db1bf4e409cc9079548fff31e76b22984c0c2d))
* dedupe fetched images by src url to prevent duplicates in image list ([430fd1b](https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/commit/430fd1b3f657a36c0570d507d17a067f8b5d48be))

## [1.7.1](https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/compare/v1.7.0...v1.7.1) (2026-01-30)


### Bug Fixes

* **download:** prevent overlay from reverting to PDF label during CBZ/ZIP download ([e2862b0](https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/commit/e2862b0ca8594c82c2d4a6fd16b067a32bfaff5b))

## [1.7.0](https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/compare/v1.6.0...v1.7.0) (2026-01-30)


### Features

* **downloads:** add CBZ and ZIP download support ([3f3ffab](https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/commit/3f3ffabc7e5026a94a04425832961ef4cf9978ed))


### Bug Fixes

* Download section CSS design fixed. ([6908594](https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/commit/69085943e1d90225231f74f820970ebad32eb41a))

## [1.6.0](https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/compare/v1.5.3...v1.6.0) (2026-01-29)


### Features

* persist applied filters; separate download progress per tab ([e0476a0](https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/commit/e0476a058e49d86fae927546e6a9bc2a6a628e9e))

## [1.5.3](https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/compare/v1.5.2...v1.5.3) (2026-01-27)


### Bug Fixes

* handle stale download state and clear on error ([7dc47cb](https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/commit/7dc47cbed3fe6eae9b8a4da5d32bad72d7b0ebd6))
* handle stale download state and clear on error ([28576cf](https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/commit/28576cffb61435fce7dd1b95ed9c39ad13dc8232))

## [1.5.2](https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/compare/v1.5.1...v1.5.2) (2026-01-15)


### Bug Fixes

* test release automation ([b178675](https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/commit/b178675b25cb6e02651f00751032ab0b91b7bf1e))
* test release automation ([ede0d25](https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/commit/ede0d253444b882e987c7a92174688807febd809))
* test release automation ([222711c](https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/commit/222711c668f28163a8f29823f52be31fd55ddd72))

## [1.5.1](https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/compare/comics-manga-downloader-v1.5.0...comics-manga-downloader-v1.5.1) (2026-01-15)


### Bug Fixes

* test release automation ([ede0d25](https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/commit/ede0d253444b882e987c7a92174688807febd809))
* test release automation ([222711c](https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/commit/222711c668f28163a8f29823f52be31fd55ddd72))

## [1.5.0](https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/compare/v1.4.10...v1.5.0) (2026-01-15)


### Features

* implement download state persistence and restoration in popup ([3aff104](https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/commit/3aff104d3cc0d453a220b2e0cde11f40cd96589d))
* implement download state persistence and restoration in popup ([97b33be](https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/commit/97b33beb219d02a3c42ee56ec6b40a7251969e26))


### Bug Fixes

* enhance CI/CD pipeline with workflow dispatch options for releas… ([7e4ffa6](https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/commit/7e4ffa6128437c928ffef4ad60fbc0e43c2fc482))
* enhance CI/CD pipeline with workflow dispatch options for release mode ([33b66e8](https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/commit/33b66e8dd33e1f5cce6421789725b4726cce4aab))

## [1.4.10](https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/compare/v1.4.9...v1.4.10) (2026-01-14)


### Bug Fixes

* refine release workflow to build assets before creating GitHub R… ([87055cf](https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/commit/87055cfc00ee88fb6dde0227f0d4968ea22c51b3))
* refine release workflow to build assets before creating GitHub Release ([2dae677](https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/commit/2dae6775d55474e22637433b18495a937bf953fd))

## [1.4.9](https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/compare/v1.4.8...v1.4.9) (2026-01-14)


### Bug Fixes

* integrate release workflow and remove obsolete files ([9ebd9cc](https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/commit/9ebd9cc163ad52d3fb65899c0009162ee9b00134))
* integrate release workflow and remove obsolete files ([00bfe77](https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/commit/00bfe77c1bf9e410b3e9748551fd39f91af566ff))
* streamline branch specification and enhance logging in release w… ([d3f28e1](https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/commit/d3f28e13ecdda87b9398e0a8f9954d413bcec9a6))
* streamline branch specification and enhance logging in release workflow ([ff5761a](https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/commit/ff5761aecd0db0149200aa0495af87b1b18877af))
* streamline release workflow and improve asset handling ([c5938c2](https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/commit/c5938c21990fc1295fab613cb6cbc0a95c6debbd))
* streamline release workflow and improve asset handling ([8ac2e52](https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/commit/8ac2e524abdfb2b264bc4d7bb7e6fe20d4e5c481))
* update pull request permissions and clean up workflow conditions ([e483997](https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/commit/e48399724c6cbfec21de2ef2232439ee598e0a61))
* update pull request permissions and clean up workflow conditions ([cc5e7cf](https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/commit/cc5e7cf82d97ce85bb544e34282e00812d066555))
* update workflow name for clarity ([9f552e9](https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/commit/9f552e9a7161acb515f2918cc3a5e63f83cc29a9))
* update workflow name for clarity ([3192115](https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/commit/319211526f7f7990706499cce19c29553d979e6f))

## [1.4.8](https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/compare/v1.4.7...v1.4.8) (2026-01-14)


### Bug Fixes

* update GitHub Actions workflows for release process ([0b66dda](https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/commit/0b66dda4e3ae4c112a206009e0c49736b14d66e8))
* update GitHub Actions workflows for release process ([f617634](https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/commit/f6176345a825e103aa6c599cef5fc87081fec71f))

## [1.4.7](https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/compare/v1.4.6...v1.4.7) (2026-01-14)


### Bug Fixes

* add release and build workflows for GitHub Actions ([719d3b7](https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/commit/719d3b7a896b0af9dec5ed6344d9dbd190f939d9))
* add release and build workflows for GitHub Actions ([65f8a06](https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/commit/65f8a069014ec1e56aa75e1f2b3d8ddcdfa7f418))

## [1.4.6](https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/compare/v1.4.5...v1.4.6) (2026-01-14)


### Bug Fixes

* integrate CI/CD pipeline with release and build workflows ([93d4ad0](https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/commit/93d4ad0800248135958fb0b32236cfc5b4eb95ba))
* integrate CI/CD pipeline with release and build workflows ([d3e34ec](https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/commit/d3e34ec8b1cb80003eb2040654453c5402d8edfd))

## [1.4.5](https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/compare/v1.4.4...v1.4.5) (2026-01-14)


### Bug Fixes

* update CI/CD workflows to trigger on release events and streamli… ([18d0bf7](https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/commit/18d0bf7724b3fbd05ee48a9ae6525457911dd838))
* update CI/CD workflows to trigger on release events and streamline asset handling ([ed258a4](https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/commit/ed258a411801a6d0c1cb4a65aa108ce4ec2ff8ac))

## [1.4.4](https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/compare/comics-manga-downloader-v1.4.3...comics-manga-downloader-v1.4.4) (2026-01-14)


### Bug Fixes

* update CI/CD workflows to trigger builds on tag pushes and strea… ([52d2b08](https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/commit/52d2b085d6b5582f6ec1b38540967a63bf70cb3c))
* update CI/CD workflows to trigger builds on tag pushes and streamline release process ([5bfba76](https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/commit/5bfba76e2454c86dd5759a09bb68b4e5192aaa12))

## [1.4.3](https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/compare/comics-manga-downloader-v1.4.2...comics-manga-downloader-v1.4.3) (2026-01-14)


### Bug Fixes

* update CI/CD workflow to streamline release tagging and asset pr… ([f5d9e01](https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/commit/f5d9e0153088c5c02fdcec4f61f75c8dd56bb986))
* update CI/CD workflow to streamline release tagging and asset preparation ([07d8603](https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/commit/07d86038c3c9fdd8360da68006a0ef87c708c738))

## [1.4.2](https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/compare/comics-manga-downloader-v1.4.1...comics-manga-downloader-v1.4.2) (2026-01-14)


### Bug Fixes

* enhance CI/CD workflow for release handling and manual dispatch ([02e3dff](https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/commit/02e3dfffe064d6dd8713d4c121be4995e2a184d8))
* enhance CI/CD workflow for release handling and manual dispatch ([628f586](https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/commit/628f586d223d0961b4d29bf2e1bb853dd0e253ac))

## [1.4.1](https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/compare/comics-manga-downloader-v1.4.0...comics-manga-downloader-v1.4.1) (2026-01-14)


### Bug Fixes

* enhance CI/CD workflows for release management ([faf1afe](https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/commit/faf1afe9b24fa9a5d415cea183989dae77b69723))
* enhance CI/CD workflows for release management ([e003bfe](https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/commit/e003bfe67d0a4ec12b892a6e0124ccff5b479f93))
* improve CI/CD workflows for release handling and debugging ([172ea5c](https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/commit/172ea5c7185a3cd6c538861e1193d19b7693d5c4))
* improve CI/CD workflows for release handling and debugging ([b522bf9](https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/commit/b522bf9906dcd97de8cd4e72515aa5dc5b5b0177))

## [1.4.0](https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/compare/comics-manga-downloader-v1.3.3...comics-manga-downloader-v1.4.0) (2026-01-14)


### Features

* Implement auto-filling of file name field based on page title and enhance filename sanitization ([77b0111](https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/commit/77b01117077833a17abc7f242ca6d6bd866dae62))
* Implement auto-filling of file name field based on page title and enhance filename sanitization ([c95185b](https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/commit/c95185bd48c6bfe51ec4e384cb31b9c2121ecae2))


### Bug Fixes

* Add bootstrap step for release-please manifest generation ([5dee47f](https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/commit/5dee47f81bd869cac5e48d14e78c6cb1ec9d2d1b))
* Add bootstrap step for release-please manifest generation ([afd6520](https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/commit/afd6520f433660ae9641cffa5cca1091ef26f7fd))
* Enhance CI/CD pipeline with release-please configuration and changelog ([ed4655a](https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/commit/ed4655a63268579f8146cb9712cdc815d2c57712))
* Enhance release process by verifying required files and updating manifest logic ([cc84de5](https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/commit/cc84de5edad32653d0a98de90775b8778abd6c7b))
* Integrate CI/CD pipeline with release automation and validation steps ([7e97ab0](https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/commit/7e97ab0e9e1f15b868c728dc48629ff9f558b61b))
* Integrate CI/CD pipeline with release automation and validation steps ([5252945](https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/commit/5252945a3d703aa1505da53e43e06528cb476c1d))
* Refactor CHANGELOG.md creation to use a more concise syntax ([7648a82](https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/commit/7648a820bcf9336f1690640944ba71635509a102))
* remove unnecessary fields from release-please configuration ([72df276](https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/commit/72df27631fc7094fa9e4726459c1d603b048c893))
* Rename bootstrap step to ensure release-please manifest exists and improve output ([1a93cef](https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/commit/1a93cef2bf50271a454ee6a8d49ac5b09f19696e))

## [1.3.3]

### Features
- Initial release

[1.3.3]: https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/releases/tag/v1.3.3
[1.3.1]: https://github.com/saptarshimondal/Comics-Manga-Downloader-Extension/releases/tag/v1.3.1
